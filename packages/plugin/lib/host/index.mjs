...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			category: {
				type: "string",
				description: "按 hub 分类过滤，如「UI 增强」"
			},
			sortBy: {
				type: "string",
				enum: [
			...
			includeExcluded: {
				type: "boolean",
				description: "是否包含被排除（占位/空仓库）条目，默认 false"
			}
		},
		output: {
			schema: {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: (value.recommendations ?? []).length === 0 ? "没有找到明显匹配的插件，试试 search_plugins 换关键词" : (value.recommendations ?? []).map((r) => `${r.fullName}（score ${r.score}）\n  理由：${r.reason}\n  ${r.description}`).join("\n")
			}]
		},
		async execute(args) {
			const doc = await loadRegistry();
			...
			return { recommendations: doc.plugins.filter((p) => !p.excluded).map((p) => {
				...
				return {
					p,
					relevance: Math.min(1, ratio) * .6 + p.score * .4,
					hits
				};
			}).filter((x) => x.hits > 0).sort((a, b) => b.relevance - a.relevance).slice(0, Math.min(args.limit ?? 5, 20)).map(({ p, hits }) => ({
				fullName: p.fullName,
				url: p.url,
				description: p.description,
				score: p.score,
