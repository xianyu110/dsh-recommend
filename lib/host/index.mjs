...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute() {
			const [regRes, hisRes] = await Promise.all([fetch(config.dataUrl), fetch(historyUrl)]);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			return { results: doc.plugins.filter((p) => (p.fullName + " " + (p.description ?? "") + " " + (p.category ?? "")).toLowerCase().includes(q)).sort((a, b) => b.score - a.score).slice(0, Math.min(args.limit ?? 10, 50)).map((p) => ({
				fullName: p.fullName,
				url: p.url,
				install: installCommand(p.fullName),
				description: p.description,
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			const tokenGroupHits = /* @__PURE__ */ new Map();
			for (const t of tokens) {
				const groups = [];
				for (const [gid, syns] of Object.entries(SYNONYM_GROUPS)) if (syns.some((s) => t.includes(s) || s.includes(t))) groups.push(gid);
			...
			return { recommendations: doc.plugins.filter((p) => !p.excluded).map((p) => {
				...
				const matchedRatio = .5 * ratioDirect + .5 * ratioGroup;
				return {
					p,
					relevance: Math.min(1, matchedRatio) * .6 + p.score * .4,
					directHits,
					groupMatched,
					expandedHits: [...expanded].filter((s) => haystack.includes(s)).length
				};
			}).filter((x) => x.directHits > 0 || x.groupMatched > 0).sort((a, b) => b.relevance - a.relevance || b.p.score - a.p.score).slice(0, Math.min(args.limit ?? 5, 20)).map(({ p, directHits, groupMatched, expandedHits }) => ({
				fullName: p.fullName,
				url: p.url,
				install: installCommand(p.fullName),
				description: p.description,
